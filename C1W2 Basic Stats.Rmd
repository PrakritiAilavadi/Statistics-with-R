...
```{r unique carriers}
unique(sfo_feb_flights$carrier)
```
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(median_ad = median(dep_delay), iqr_ad = IQR(dep_delay), count_ad = n())
```
...
nyc_flights_avg_speed <- nycflights %>%
  mutate(avg_speed = distance/air_time) %>%
  arrange(desc(avg_speed))

ggplot(data = nyc_flights_avg_speed, aes(x=avg_speed, y=distance)) + geom_point()
```
...
nyc_on_time <- nycflights %>%
  mutate(arr_type = ifelse(arr_delay <=0 , "on time", "delayed"),
         dep_type = ifelse(dep_delay<5, "on time", "delayed"))

nyc_dep <- nyc_on_time %>%
  summarise(arr_ontime = sum(arr_type == "on time")/sum(dep_type == "delayed"),
            arr_delayed = sum(arr_type == "delayed")/sum(dep_type == "delayed"))

nyc_dep %>%
  mutate(x = arr_ontime/(arr_ontime + arr_delayed))
```
